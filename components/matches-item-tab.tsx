                ...
                {mainTabs.map((tab, index) => (
                    <button
                        ...
                        className={`flex-1 px-4 py-2 text-center text-sm font-semibold transition-all cursor-pointer ${activeMainTab === index
                            ? "bg-[#C1AA66] text-black rounded-lg"
                            : "text-gray-500"
                            }`}
                    >
                        {tab}
            ...
            {activeMainTab === 1 && (
                <>
                    <div className="flex justify-between mt-4 text-sm font-semibold">
                        {subTabs.map((tab, index) => (
                            <button
                                ...
                                className={`bg-gray-800 text-white p-4 w-[30%]  transition-all cursor-pointer hover:bg-gray-800/90  ${activeSubTab === index ? "text-black" : "text-gray-500"
                                    }`}
                            >
                                {tab}
                            </button>
                    ...
                    <div className="relative mt-1 h-1 bg-gray-300">
                        <div
                            className="absolute h-1 bg-[#C1AA66] transition-all"
                            style={{
                                width: "50%",
